...
pub struct Vl53l7cx<B: BusOperation, LPN: OutputPin, T: DelayNs> {
    pub(crate) temp_buffer: [u8;  VL53L7CX_TEMPORARY_BUFFER_SIZE],
    pub(crate) offset_data: [u8;  VL53L7CX_OFFSET_BUFFER_SIZE],
    pub(crate) xtalk_data: [u8; VL53L7CX_XTALK_BUFFER_SIZE],
...
/// Structure ResultsData contains the ranging results of
 /// VL53L7CX. If user wants more than 1 target per zone, 
...
pub struct ResultsData {
  // Internal sensor silicon temperature 
    ...
    #[cfg(not(feature="VL53L7CX_DISABLE_AMBIENT_PER_SPAD"))]
  // Ambient noise in kcps/spads 
  pub ambient_per_spad: [u32; VL53L7CX_RESOLUTION_8X8 as usize],
    #[cfg(not(feature="VL53L7CX_DISABLE_NB_TARGET_DETECTED"))]
  // Number of valid target detected for 1 zone 
  pub nb_target_detected: [u8; VL53L7CX_RESOLUTION_8X8 as usize],
    #[cfg(not(feature="VL53L7CX_DISABLE_NB_SPADS_ENABLED"))]
  // Number of spads enabled for this ranging 
    pub nb_spads_enabled: [u32; VL53L7CX_RESOLUTION_8X8 as usize],
    #[cfg(not(feature="VL53L7CX_DISABLE_SIGNAL_PER_SPAD"))]
  // Signal returned to the sensor in kcps/spads 
    pub signal_per_spad: [u32; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
    #[cfg(not(feature="VL53L7CX_DISABLE_RANGE_SIGMA_MM"))]
  // Sigma of the current distance in mm 
    pub range_sigma_mm: [u16; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
    #[cfg(not(feature="VL53L7CX_DISABLE_DISTANCE_MM"))]
  // Measured distance in mm 
    pub distance_mm: [i16; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
    #[cfg(not(feature="VL53L7CX_DISABLE_REFLECTANCE_PERCENT"))]
  // Estimated reflectance in percent 
    pub reflectance: [u8; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
    #[cfg(not(feature="VL53L7CX_DISABLE_TARGET_STATUS"))]
  // Status indicating the measurement validity (5 & 9 means ranging OK)
    pub target_status: [u8; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
    #[cfg(not(feature="VL53L7CX_DISABLE_MOTION_INDICATOR"))]
...
impl ResultsData {
    pub fn new() -> Self {
        ResultsData {
            silicon_temp_degc: 0, 
            #[cfg(not(feature="VL53L7CX_DISABLE_AMBIENT_PER_SPAD"))]
            ambient_per_spad: [0; VL53L7CX_RESOLUTION_8X8 as usize],
            #[cfg(not(feature="VL53L7CX_DISABLE_NB_TARGET_DETECTED"))]
            nb_target_detected: [0; VL53L7CX_RESOLUTION_8X8 as usize],
            #[cfg(not(feature="VL53L7CX_DISABLE_NB_SPADS_ENABLED"))]
            nb_spads_enabled: [0; VL53L7CX_RESOLUTION_8X8 as usize],
            #[cfg(not(feature="VL53L7CX_DISABLE_SIGNAL_PER_SPAD"))]
            signal_per_spad: [0; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
            #[cfg(not(feature="VL53L7CX_DISABLE_RANGE_SIGMA_MM"))]
            range_sigma_mm: [0; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
            #[cfg(not(feature="VL53L7CX_DISABLE_DISTANCE_MM"))]
            distance_mm: [0; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
            #[cfg(not(feature="VL53L7CX_DISABLE_REFLECTANCE_PERCENT"))]
            reflectance: [0; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
            #[cfg(not(feature="VL53L7CX_DISABLE_TARGET_STATUS"))]
            target_status: [0; (VL53L7CX_RESOLUTION_8X8 as usize) * (VL53L7CX_NB_TARGET_PER_ZONE as usize)],
            #[cfg(not(feature="VL53L7CX_DISABLE_MOTION_INDICATOR"))]
...
impl<B: BusOperation, LPN: OutputPin, T: DelayNs> Vl53l7cx<B, LPN, T> {
    ...
    /// This function is used to wait for an answer from VL53L7CX sensor.
    ...
    pub(crate) fn send_offset_data(&mut self, resolution: u8) -> Result<(), Error<B::Error>> {
        ...
        self.temp_buffer[..VL53L7CX_OFFSET_BUFFER_SIZE].copy_from_slice(&self.offset_data);

        // Data extrapolation is required for 4X4 offset 
        if resolution == VL53L7CX_RESOLUTION_4X4 {
            self.temp_buffer[0x10..0x10+dss_4x4.len()].copy_from_slice(&dss_4x4);
            swap_buffer(&mut self.temp_buffer, VL53L7CX_OFFSET_BUFFER_SIZE);
        ...
        for i in 0..VL53L7CX_OFFSET_BUFFER_SIZE-4 {
            self.temp_buffer[i] = self.temp_buffer[i+8];
        }

        self.temp_buffer[0x1E0..0x1E0+footer.len()].copy_from_slice(&footer);
        self.write_multi_to_register_temp_buffer(0x2E18, VL53L7CX_OFFSET_BUFFER_SIZE)?;
        self.poll_for_answer(4, 1, VL53L7CX_UI_CMD_STATUS, 0xFF, 0x03)?;
    ...
    pub(crate) fn send_xtalk_data(&mut self, resolution: u8) -> Result<(), Error<B::Error>> {
        ...
        self.temp_buffer[..VL53L7CX_XTALK_BUFFER_SIZE].copy_from_slice(&self.xtalk_data);
        ...
        if resolution == VL53L7CX_RESOLUTION_4X4 {
            ...
            swap_buffer(&mut self.temp_buffer, VL53L7CX_XTALK_BUFFER_SIZE);
        ...
        self.write_multi_to_register_temp_buffer(0x2CF8, VL53L7CX_XTALK_BUFFER_SIZE)?;
        self.poll_for_answer(4, 1, VL53L7CX_UI_CMD_STATUS, 0xFF, 0x03)?;
    ...
    /// Check if the VL53L7CX sensor is alive (responding to communication).
    ...
    pub(crate) fn dci_read_data(&mut self, index: u16, data_size: usize) -> Result<(), Error<B::Error>> {
        ...
        if read_size > VL53L7CX_TEMPORARY_BUFFER_SIZE {
            return Err(Error::Other);
        } 
        cmd[0] = (index >> 8) as u8;
        ...
        self.write_multi_to_register(VL53L7CX_UI_CMD_END - 11, &cmd)?;
        self.poll_for_answer(4, 1, VL53L7CX_UI_CMD_STATUS, 0xFF, 0x03)?;
        
        // Read new data sent (4 bytes header + data_size + 8 bytes footer) 
        self.read_from_register(VL53L7CX_UI_CMD_START, read_size)?;
    ...
    pub(crate) fn dci_write_data(&mut self, index: u16, data_size: usize) -> Result<(), Error<B::Error>> {
        ...
        let address: u16 = VL53L7CX_UI_CMD_END - (data_size as u16 + 12) + 1;

        // Check if cmd buffer is large enough 
        if (data_size + 12) > VL53L7CX_TEMPORARY_BUFFER_SIZE {
            return Err(Error::Other);
        } else {
    ...
    /// to load the firmware into the VL53L7CX. 
    ...
    pub fn init(&mut self) -> Result<(), Error<B::Error>> {
        ...
        self.write_multi_to_register(0, &VL53L7CX_FIRMWARE[0..0x8000])?;
        ...
        self.send_xtalk_data(VL53L7CX_RESOLUTION_4X4)?;
        ...
        self.dci_write_data(VL53L7CX_DCI_PIPE_CONTROL, 4)?;
      
        if VL53L7CX_NB_TARGET_PER_ZONE != 1 {
            let tmp: [u8; 1] = [VL53L7CX_NB_TARGET_PER_ZONE as u8];
            self.dci_replace_data(VL53L7CX_DCI_FW_NB_TARGET, 16, &tmp, 1, 0x0C)?;
        }
        from_u32_to_u8(&single_range, &mut self.temp_buffer[..4]);
        self.dci_write_data( VL53L7CX_DCI_SINGLE_RANGE, 4)?;
    ...
    pub fn start_ranging(&mut self) -> Result<(), Error<B::Error>> {
        ...
        if !cfg!(feature = "VL53L7CX_DISABLE_AMBIENT_PER_SPAD") { output_bh_enable[0] += 8; }
        if !cfg!(feature = "VL53L7CX_DISABLE_NB_SPADS_ENABLED") { output_bh_enable[0] += 16; }
        if !cfg!(feature = "VL53L7CX_DISABLE_NB_TARGET_DETECTED") { output_bh_enable[0] += 32; }
        if !cfg!(feature = "VL53L7CX_DISABLE_SIGNAL_PER_SPAD") { output_bh_enable[0] += 64; }
        if !cfg!(feature = "VL53L7CX_DISABLE_RANGE_SIGMA_MM") { output_bh_enable[0] += 128; }
        if !cfg!(feature = "VL53L7CX_DISABLE_DISTANCE_MM") { output_bh_enable[0] += 256; }
        if !cfg!(feature = "VL53L7CX_DISABLE_REFLECTANCE_PERCENT") { output_bh_enable[0] += 512; }
        if !cfg!(feature = "VL53L7CX_DISABLE_TARGET_STATUS") { output_bh_enable[0] += 1024; }
        if !cfg!(feature = "VL53L7CX_DISABLE_MOTION_INDICATOR") { output_bh_enable[0] += 2048; }
        ...
        for i in 0..12 {
            ...
            if bh.bh_type() >= 0x01 && bh.bh_type() < 0x0d {
                if bh.bh_idx() >= 0x54d0 && bh.bh_idx() < 0x54d0 + 960 {
                    ...
                    bh.set_bh_size(resolution as u32 * VL53L7CX_NB_TARGET_PER_ZONE);}
    ...
    pub fn get_ranging_data(&mut self) -> Result<ResultsData, Error<B::Error>> {
        ...
        while i < self.data_read_size as usize {
            ...
            if bh.bh_idx() == VL53L7CX_METADATA_IDX as u32 {
            ...
            let mut src: &[u8] = &[0];
            if i+msize <= VL53L7CX_TEMPORARY_BUFFER_SIZE {
            ...
            #[cfg(not(feature = "VL53L7CX_DISABLE_AMBIENT_PER_SPAD"))] 
            if bh.bh_idx() == VL53L7CX_AMBIENT_RATE_IDX as u32 {
                from_u8_to_u32(src, &mut result.ambient_per_spad);
                continue;
            }

            #[cfg(not(feature = "VL53L7CX_DISABLE_NB_SPADS_ENABLED"))] 
            if bh.bh_idx() == VL53L7CX_SPAD_COUNT_IDX as u32 {
                from_u8_to_u32(src, &mut result.nb_spads_enabled);
                continue;
            }

            #[cfg(not(feature = "VL53L7CX_DISABLE_NB_TARGET_DETECTED"))] 
            if bh.bh_idx() == VL53L7CX_NB_TARGET_DETECTED_IDX as u32 {
                result.nb_target_detected[..msize].copy_from_slice(src); 
                continue;
            }

            #[cfg(not(feature = "VL53L7CX_DISABLE_SIGNAL_PER_SPAD"))]
            if bh.bh_idx() == VL53L7CX_SIGNAL_RATE_IDX as u32 {
                from_u8_to_u32(src, &mut result.signal_per_spad);
                continue;
            }

            #[cfg(not(feature = "VL53L7CX_DISABLE_RANGE_SIGMA_MM"))]
            if bh.bh_idx() == VL53L7CX_RANGE_SIGMA_MM_IDX as u32 {
                from_u8_to_u16(src, &mut result.range_sigma_mm);   
                continue;
            } 

            #[cfg(not(feature = "VL53L7CX_DISABLE_DISTANCE_MM"))] 
            if bh.bh_idx() == VL53L7CX_DISTANCE_IDX as u32 {
                from_u8_to_i16(src, &mut result.distance_mm);
                continue;
            }

            #[cfg(not(feature= "VL53L7CX_DISABLE_REFLECTANCE_PERCENT"))]
            if bh.bh_idx() == VL53L7CX_REFLECTANCE_EST_PC_IDX as u32 {
                result.reflectance[..msize].copy_from_slice(src); 
                continue;
            }

            #[cfg(not(feature = "VL53L7CX_DISABLE_TARGET_STATUS"))]
            if bh.bh_idx() == VL53L7CX_TARGET_STATUS_IDX as u32 {
                result.target_status[..msize].copy_from_slice(src); 
                continue;
            }

            #[cfg(not(feature = "VL53L7CX_DISABLE_MOTION_INDICATOR"))]
            if bh.bh_idx() == VL53L7CX_MOTION_DETEC_IDX as u32 {
                from_u8_to_motion_indicator(src, &mut result.motion_indicator);
                continue;
            }
        }
        if VL53L7CX_USE_RAW_FORMAT == 0 {
            // Convert data into their real format 
            #[cfg(not(feature = "VL53L7CX_DISABLE_AMBIENT_PER_SPAD"))] {
                for i in 0..VL53L7CX_RESOLUTION_8X8 as usize {
                    result.ambient_per_spad[i] /= 2048;
                }
            }
            for i in 0..(VL53L7CX_RESOLUTION_8X8 as usize)*(VL53L7CX_NB_TARGET_PER_ZONE as usize) {
                #[cfg(not(feature = "VL53L7CX_DISABLE_DISTANCE_MM"))] {
                ...
                #[cfg(not(feature = "VL53L7CX_RANGE_SIGMA_MM"))]{
                ...
                #[cfg(not(any(feature="VL53L7CX_DISABLE_DISTANCE_MM", feature="VL53L7CX_DISABLE_TARGET_STATUS")))] {
                    for i in 0..VL53L7CX_RESOLUTION_8X8 as usize {
                        if result.nb_target_detected[i] == 0 {
                            for j in 0..VL53L7CX_NB_TARGET_PER_ZONE as usize {
                                result.target_status[VL53L7CX_NB_TARGET_PER_ZONE as usize*i + j] = 255;
                            }
                        }
                    }
                }
