...
use crate::{consts, utils, BusOperation, Vl53l7cx, Error, OutputPin, DelayNs};

/// Structure DetectionThresholds contains a single threshold. This structure  is never used alone, it must be used as an array of 64 thresholds (defined by macro VL53L7CX_NB_THRESHOLDS).
#[repr(C)]
#[derive(Clone, Copy)]
...
impl DetectionThresholds {
    pub fn new() -> Self {
        DetectionThresholds { param_low_thresh: 0,
            param_high_thresh: 0,
            measurement: VL53L7CX_DISTANCE_MM,
            th_type: VL53L7CX_IN_WINDOW,
            zone_num: 0,
            math_op: VL53L7CX_OPERATION_NONE }
    }
}
...
impl<B: BusOperation, LPN: OutputPin, T: DelayNs> Vl53l7cx<B, LPN, T> {
    ...
    pub fn get_detection_thresholds_enable(&mut self) -> Result<u8, Error<B::Error>> {
        let enabled: u8;
        self.dci_read_data(VL53L7CX_DCI_DET_THRESH_GLOBAL_CONFIG, 8)?;
    ...
    pub fn set_detection_thresholds_enable(&mut self, enabled: u8) -> Result<(), Error<B::Error>> {
        ...
        self.dci_replace_data(VL53L7CX_DCI_DET_THRESH_GLOBAL_CONFIG, 8, &grp_global_config, 4, 0x00)?;
        
        // Update interrupt config 
        self.dci_replace_data(VL53L7CX_DCI_DET_THRESH_CONFIG, 20, &tmp, 1, 0x11)?;
    ...
    pub fn get_detection_thresholds(&mut self) -> Result<[DetectionThresholds; VL53L7CX_NB_THRESHOLDS], Error<B::Error>> {
        let mut thresholds: [DetectionThresholds; VL53L7CX_NB_THRESHOLDS] = [DetectionThresholds::new(); VL53L7CX_NB_THRESHOLDS];
        
        // Get thresholds configuration 
        self.dci_read_data(VL53L7CX_DCI_DET_THRESH_START, VL53L7CX_NB_THRESHOLDS * 12)?;
        from_u8_to_thresholds(&self.temp_buffer[..VL53L7CX_NB_THRESHOLDS * 12], &mut thresholds);
        
        for i in 0..VL53L7CX_NB_THRESHOLDS {
            if thresholds[i].measurement == VL53L7CX_DISTANCE_MM {
                thresholds[i].param_low_thresh  /= 4;
                thresholds[i].param_high_thresh /= 4;
            } else if thresholds[i].measurement == VL53L7CX_SIGNAL_PER_SPAD_KCPS {
                thresholds[i].param_low_thresh  /= 2048;
                thresholds[i].param_high_thresh /= 2048;
            } else if thresholds[i].measurement == VL53L7CX_RANGE_SIGMA_MM {
                thresholds[i].param_low_thresh  /= 128;
                thresholds[i].param_high_thresh /= 128;
            } else if thresholds[i].measurement == VL53L7CX_AMBIENT_PER_SPAD_KCPS {
                thresholds[i].param_low_thresh  /= 2048;
                thresholds[i].param_high_thresh /= 2048;
            } else if thresholds[i].measurement == VL53L7CX_NB_SPADS_ENABLED {
                thresholds[i].param_low_thresh  /= 256;
                thresholds[i].param_high_thresh /= 256;
            } else if thresholds[i].measurement == VL53L7CX_MOTION_INDICATOR {
    ...
    /// 
    /// * `thresholds` :  Array of 64 thresholds.
    pub fn set_detection_thresholds(&mut self, thresholds: &mut [DetectionThresholds; VL53L7CX_NB_THRESHOLDS] ) -> Result<(), Error<B::Error>> {
        for i in 0..VL53L7CX_NB_THRESHOLDS {
            if thresholds[i].measurement == VL53L7CX_DISTANCE_MM {
                thresholds[i].param_low_thresh  *= 4;
                thresholds[i].param_high_thresh *= 4;
        ...
        self.dci_write_data(VL53L7CX_DCI_DET_THRESH_VALID_STATUS, 8)?;
        
        // Set thresholds configuration 
        from_thresholds_to_u8(thresholds, &mut self.temp_buffer[..VL53L7CX_NB_THRESHOLDS * 12]);
        self.dci_write_data(VL53L7CX_DCI_DET_THRESH_START, VL53L7CX_NB_THRESHOLDS * 12)?;
    ...
    pub fn get_detection_thresholds_auto_stop(&mut self) -> Result<bool, Error<B::Error>> {
        self.dci_read_data(VL53L7CX_DCI_PIPE_CONTROL, 4)?;
    ...
    pub fn set_detection_thresholds_auto_stop(&mut self, auto_stop: bool) -> Result<(), Error<B::Error>> {
        ...
        self.dci_replace_data(VL53L7CX_DCI_PIPE_CONTROL, 4, &tmp, 1, 0x03)?;
